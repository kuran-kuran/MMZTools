...
;================================================
;LZEデータをデコードする。
;in	HL	デコード前データへのポインタ（ダミーGET_BYTE使用時）
;	DE	デコード後データへのポインタ
;圧縮入力はすべてGET_BYTEを経由する。
;================================================
...
DECODE_LZE_lp0:
DECODE_LZE_1:
	CALL	GET_BYTE	;*(DECODEPtr++) = GET_BYTE();
	EXX
...
DECODE_LZE_lp1:
	...
	CALL	GET_BYTE	;	BitData = GET_BYTE();
	LD	C,A
	LD	B,8	;	BitCount = 8;
...
SKIP01:	SLA	C	;	BitData <<= 1;
	CALL	GET_BYTE	;	Distance = -256+GET_BYTE();
	EXX
	LD	L,A
