...
def default_movie_filename(input_folder: Path) -> str:
    """Build the default joined filename from the selected PNG folder."""
    return f"{input_folder.name}.mzt"
...
def copy_final_outputs(sources: list[Path], destination_folder: Path) -> list[Path]:
    """Copy completed movie files beside the dropped PNG folder."""
    copied: list[Path] = []
    for source in sources:
...
def remove_encoded_folder(folder: Path) -> bool:
    """Remove the conventional generated-output folder, never an arbitrary path."""
    if folder.name.casefold() != "encoded" or not folder.is_dir():
        return False
...
def encode_folder(
    ...
    *,
    ...
    if not cancelled.is_set():
        ...
        copied_paths = copy_final_outputs(
            [movie_path, combined_path], input_folder.parent
        )
        for path in copied_paths:
            report("log", f"{path.name}  →  {path.parent}へコピー")
        if not keep_parts:
            ...
            if remove_encoded_folder(output_folder):
                report("log", f"{output_folder}を削除しました")
...
class EncoderGui:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.concatenate_name_var = tk.StringVar()
    ...
    def _set_input(self, folder: Path) -> None:
        ...
        self.concatenate_name_var.set(default_movie_filename(folder))
